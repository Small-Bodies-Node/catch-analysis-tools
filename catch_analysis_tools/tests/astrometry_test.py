...
def test_run_solve_field_conditional_execution(file_exists, should_call_run):
    with (
        patch.dict(os.environ, {"ASTROMETRY_CONFIG": "/fake/astrometry/config"}),
        patch(
            "catch_analysis_tools.astrometry.os.path.exists", return_value=file_exists
        ) as mock_exists,
...
def test_load_wcs(synthetic_wcs):
    with tempfile.NamedTemporaryFile(suffix=".fits", delete=False) as tmp:
        filename = tmp.name
    try:
        fits.PrimaryHDU(header=synthetic_wcs.to_header()).writeto(
